...
for (let i = 1; i <= LAST_MIGRATION; i++) {
  ...
  Migrations.add(
      {version : i, up() { migration.up(); }, down() { migration.down(); }});
}
...
function findLatestVersion() {
  ...
  Migrations._list.forEach((entry) => { max = Math.max(entry.version, max); });
...
export const handleMigration = function() {
  ...
  if (currentVersion < latestVersion) {
    let basePath = Meteor.settings && Meteor.settings.db &&
                       Meteor.settings.db.mongodumpTargetDirectory ||
                   '';

    if (basePath !== '') {
      let backupPath =
          path.join(basePath, `mongobackup_${moment().format('YYYY-MM-DD')}`);
