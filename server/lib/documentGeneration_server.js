...
convertHTML2PDF = (htmldata, fileName, metaData) => {
  ...
  let checkFileExists = (filepath, fileNameForErrorMsg) => {
    if (!fs.existsSync(filepath)) {
      throw new Meteor.Error(
          "runtime-error",
          "Error at PDF generation: " + fileNameForErrorMsg +
  ...
  let outputPath = `${
      getDocumentStorageRootDirectory()}/TemporaryProtocol.pdf`; // eslint-disable-line

  let additionalArguments = "";
  if (Meteor.settings.docGeneration.wkhtmltopdfParameters &&
  ...
  exec(
      exePath + additionalArguments + ' "' + tempFileName + '" "' + outputPath +
          '"',
      {
  ...
  if (Meteor.settings.public.docGeneration.format === "pdfa") {
    ...
    outputPath = `${
        getDocumentStorageRootDirectory()}/TemporaryProtocol-A.pdf`; // eslint-disable-line
    additionalArguments =
    ...
    try {
      exec(
          exePath + additionalArguments + '"' + outputPath + '" "' +
              Meteor.settings.docGeneration.pathToPDFADefinitionFile + '" "' +
              inputPath + '"',
          {
