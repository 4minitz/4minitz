...
class ExpImpTopics {
  static get FILENAME_POSTFIX() { return "_topics.json"; }

  static doExport(db, msID, userIDs) {
    return new Promise((resolve, reject) => {
      db.collection("topics").find({parentId : msID}).toArray().then((doc) => {
        if (doc) {
  ...
  static doImport(db, msID, usrMap) {
    return new Promise((resolve, reject) => {
      ...
      return db.collection("topics")
          .deleteMany(
              {_id : {$in : topicIDs}}) // delete existing topics with same IDs
          .then((res) => {
            ...
            return db.collection("topics")
                ...
                .then((res) => {
                  if (res.result.ok === 1 &&
                      res.result.n === AllTopicsDoc.length) {
  ...
  static patchUsers(topicDoc, usrMap) {
    ...
    for (let i = 0; topicDoc.responsibles && i < topicDoc.responsibles.length;
         i++) {
    ...
    for (let i = 0; topicDoc.infoItems && i < topicDoc.infoItems.length; i++) {
      for (let j = 0; topicDoc.infoItems[i].responsibles &&
