...
class ExpImpMinutes {
  static get FILENAME_POSTFIX() { return "_minutes.json"; }
      ...
      db.collection("minutes")
          ...
          .then((allMinutesDoc) => {
            if (allMinutesDoc) {
              ...
              fs.writeFileSync(minFile,
                               EJSON.stringify(allMinutesDoc, null, 2));
              console.log(
                  "Saved: " + minFile + " with " + allMinutesDoc.length +
                      " minutes",
              );
              ...
              allMinutesDoc.map((min) => {
                ...
                min.topics?.map((top) => {
                  ...
                  top.infoItems?.map((item) => {
                    ...
                    item.responsibles?.map((resp) => { userIDs[resp] = 1; });
                  });
                });
              });
  ...
  static doImport(db, msID, usrMap) {
    return new Promise((resolve, reject) => {
      ...
      return db.collection("minutes")
          .deleteMany(
              {_id : {$in : minIDs}}) // delete existing minutes with same IDs
          .then(function(res) {
  ...
  static patchUsers(minDoc, usrMap) {
    ...
    for (let m = 0; m < minDoc.length; m++) {
      ...
      for (let i = 0; minDoc[m].visibleFor && i < minDoc[m].visibleFor.length;
           i++) {
        minDoc[m].visibleFor[i] = usrMap[minDoc[m].visibleFor[i]];
      }
      for (let i = 0;
           minDoc[m].informedUsers && i < minDoc[m].informedUsers.length; i++) {
        minDoc[m].informedUsers[i] = usrMap[minDoc[m].informedUsers[i]];
      }
      for (let i = 0;
           minDoc[m].participants && i < minDoc[m].participants.length; i++) {
