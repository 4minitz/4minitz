...
class ExpImpMinutes {
  ...
  static patchUsers(minDoc, usrMap) {
    ...
    for (const element of minDoc) {
      ...
      for (let i = 0; element.visibleFor && i < element.visibleFor.length;
           i++) {
        element.visibleFor[i] = usrMap[element.visibleFor[i]];
      }
      for (let i = 0; element.informedUsers && i < element.informedUsers.length;
           i++) {
        element.informedUsers[i] = usrMap[element.informedUsers[i]];
      }
      for (let i = 0; element.participants && i < element.participants.length;
           i++) {
