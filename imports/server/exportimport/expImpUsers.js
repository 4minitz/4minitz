...
class ExpImpUsers {
  static get FILENAME_POSTFIX() { return "_users.json"; }
  static get MAPNAME_POSTFIX() { return "_userMap.json"; }
      ...
      db.collection("users")
          ...
          .then((allUsersDoc) => {
            if (allUsersDoc) {
              ...
              // from DB
              const userIDsFromDB = {};
              allUsersDoc.map((usr) => { userIDsFromDB[usr._id] = 1; });
              ...
              fs.writeFileSync(mapFile,
                               JSON.stringify(userIDsOuputMap, null, 2));
  ...
  static preImportCheck(db, msID) {
    return new Promise((resolve, reject) => {
      ...
      usrMap = Object.keys(usrMap).map(
          (key) => { usrMapSimple[key] = usrMap[key].newID; });
      usrMap = usrMapSimple;
      ...
      db.collection("users")
          ...
          .then((doc) => {
            if (doc) {
              ...
              console.log(
                  "Will copy over " + usrCopyIDs.length +
    ...
    return new Promise((resolve, reject) => {
      ...
      for (let u = 0; u < allUsersDoc.length; u++) {
        if (allUsersDoc[u]._id === usrMap[allUsersDoc[u]._id]) {
          ...
          const roleValueForMS =
              allUsersDoc[u].roles[msID]; // Case#1: clone this user from source
          // DB => target DB!
          allUsersDoc[u].roles = {
            msID : roleValueForMS,
