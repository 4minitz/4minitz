    ...
    static doImport (db, msID, usrMap) {
        return new Promise((resolve, reject) => {
            ...
            for(let p=0; p<AllProtocolsDoc.length; p++) {
                ...
                AllProtocolsDoc[p] = ExpImpFilesDocuments.patchUsers(AllProtocolsDoc[p], usrMap);
            }


            return db.collection('DocumentsCollection')
                ...
                .then(function (res) {
                    ...
                    return db.collection('DocumentsCollection')
                        ...
                        .then(function (res) {
                            if (res.result.ok === 1 && res.result.n === AllProtocolsDoc.length) {
                                console.log('OK, inserted '+res.result.n+' protocol files meta data.');
                                resolve({db, usrMap});
                            } else {
