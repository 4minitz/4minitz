...
class ExpImpMeetingSeries {
  static get FILENAME_POSTFIX() { return "_meetingSeries.json"; }

  static doExport(db, msID) {
    return new Promise((resolve, reject) => {
      const userIDs = {};
      db.collection("meetingSeries").findOne({_id : msID}).then((doc) => {
        if (doc) {
          ...
          doc.visibleFor?.map((userID) => { userIDs[userID] = 1; });
          doc.informedUsers?.map((userID) => { userIDs[userID] = 1; });
          resolve({db, userIDs});
          return;
        }
  ...
  static doImport(db, msID, usrMap) {
    return new Promise((resolve, reject) => {
      db.collection("meetingSeries").findOne({_id : msID}).then((doc) => {
        ...
        return db.collection("meetingSeries").insert(msDoc).then((res) => {
          if (res.result.ok === 1) {
