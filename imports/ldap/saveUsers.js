let mongo = require('mongodb').MongoClient,
    ...
    transformUser = require('./transformUser'), _ = require('underscore');
import {Random} from '../../tests/performance/fixtures/lib/random';

let _transformUsers = function(
    settings,
    users) { return _.map(users, user => transformUser(settings, user)); };

let _connectMongo = function(mongoUrl) { return mongo.connect(mongoUrl); };
...
let _insertUsers = function(client, mongoUri, users) {
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      let bulk = client.db(mongoConnection.database)
                     .collection('users')
      ...
      _.each(users, user => {
        if (user && user.username && user.emails[0] && user.emails[0].address) {
          ...
          let usrRegExp =
              new RegExp('^' + RegExp.escape(user.username) + '$', 'i');
          bulk.find({username : usrRegExp}).upsert().updateOne({
            $setOnInsert : {
              ...
              services :
                  {password : {bcrypt : ''}, resume : {'loginTokens' : []}}
            },
            $set : user
          });
        } else {
...
let _closeMongo = function(data) {
  let force = false, client = data.client, result = data.bulkResult;
...
let saveUsers = function(settings, mongoUrl, users) {
  ...
  return new Promise((resolve, reject) => {
    _connectMongo(mongoUrl)
        .then(client => { return _insertUsers(client, mongoUrl, dbUsers); })
        .then(_closeMongo)
        .then(resolve)
        .catch(reject);
  });
