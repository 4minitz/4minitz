...
export class TopicsFilter {
  ...
  docMatchesSearchTokens(doc, searchTokens) {
    for (let i = 0; i < searchTokens.length; i++) {
      ...
      const hasMatchingInfoItems =
          this.itemsFilter
              .filterWithParams(doc.infoItems, this.isCaseSensitive,
                                [
                                  token,
                                ])
              .length > 0;
  ...
  _toUpper(str) { return this.isCaseSensitive ? str : str.toUpperCase(); }

  docMatchesLabelTokens(doc, labelTokens) {
    for (let i = 0; i < labelTokens.length; i++) {
      const token = labelTokens[i];
      const hasMatchingInfoItems = this.itemsFilter
                                       .filterWithParams(
                                           doc.infoItems,
