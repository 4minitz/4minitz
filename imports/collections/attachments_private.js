...
export let AttachmentsCollection = new FilesCollection({
  ...
  storagePath : Meteor.isServer ? calculateAndCreateStoragePath
  ...
  onBeforeUpload : function(file) {
    ...
    if (Meteor.settings.public.attachments.denyExtensions !== undefined) {
      ...
      if (denyRE.test(file.extension) || forbiddenRE.test(file.extension)) {
        return ('Denied file extension: "' + file.extension +
  ...
  onAfterUpload : function(file) {
    console.log(
        "Successfully uploaded attachment file: " + file.name + " to " +
            file.path,
    );
