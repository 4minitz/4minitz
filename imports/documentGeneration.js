...
export class DocumentGeneration {
  ...
  static async downloadMinuteProtocol(minuteID, noProtocolExistsDialog) {
    ...
    let generateAndDownloadHTML = async function() {
      ...
      const filename = `${
          DocumentGeneration.calcFileNameforMinute(
              currentMinute,
              )}.html`;
      ...
      if (window.navigator.msSaveOrOpenBlob) {
        ...
        setTimeout(function() { window.URL.revokeObjectURL(fileurl); }, 250);
      }
    };
  ...
  static getDocumentData(context) {
    let presentParticipants = context._participants.filter(
        (participant) => { return participant.present; });

    let absentParticipants = context._participants.filter(
        (participant) => { return !participant.present; });

    let discussedTopics =
        context._topics.filter((topic) => { return !topic.isOpen; });

    let outstandingTopics = context._topics.filter(
        (topic) => { return topic.isOpen && !topic.isSkipped; });
  ...
  static addHelperForHTMLMail(template, context) {
    template.addHelper("hasLabels",
                       function() { return this.labels.length > 0; });
    ...
    template.addHelper("isActionItem",
                       function() { return this.itemType === "actionItem"; });
  }
}
