...
export class DocumentGeneration {
  ...
  static getDocumentData(context) {
    const presentParticipants = context._participants.filter(
        (participant) => { return participant.present; });

    const absentParticipants = context._participants.filter(
        (participant) => { return !participant.present; });

    const discussedTopics =
        context._topics.filter((topic) => { return !topic.isOpen; });

    const outstandingTopics = context._topics.filter(
