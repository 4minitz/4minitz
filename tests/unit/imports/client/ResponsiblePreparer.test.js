...
import {
  ParticipantsPreparer
} from '../../../../imports/client/ParticipantsPreparer';

let generateId = () => { return faker.string.uuid(); };
...
describe('ParticipantsPreparer', function() {
  let preparer, fakeMinutes, fakeParentSeries, fakeTopicOrItem,
      fakeUserCollection;
  ...
  beforeEach(function() {
    ...
    fakeTopicOrItem = {
      ...
      hasResponsibles :
          function() { return this._topicDoc.responsibles.length > 0; },
      getResponsibles : function() { return this._topicDoc.responsibles; }
    };
    fakeUserCollection = {
      ...
      find : function(selector) {
        ...
        let result = this.users.filter(
            user => {return !_.contains(excludeIds, user._id)});
        return {
          fetch: () => { return result; }
        }
      },
    ...
    preparer = new ParticipantsPreparer(fakeMinutes, fakeTopicOrItem,
                                        fakeUserCollection);
    preparer._init();
  });
  ...
  describe('#getPossibleResponsibles', function() {
    ...
    it('returns the additional responsible, too', function() {
      fakeMinutes.participantsAdditional =
          `${ADDITIONAL_RESP_TEXT}, ${ADDITIONAL_RESP_MAIL}`;
      preparer._prepareResponsibles();
      let result = preparer.getPossibleResponsibles();
      ...
      expect(result).to.deep.include(
          {id : ADDITIONAL_RESP_MAIL, text : ADDITIONAL_RESP_MAIL});
    });

    it('returns the former responsible, too', function() {
      fakeParentSeries.additionalResponsibles =
          [ FORMER_RESP_TEXT, FORMER_RESP_MAIL ];
      preparer._prepareResponsibles();
      let result = preparer.getPossibleResponsibles();
      expect(result).to.have.length(4);
      expect(result).to.deep.include(
          {id : FORMER_RESP_TEXT, text : FORMER_RESP_TEXT});
      expect(result).to.deep.include(
          {id : FORMER_RESP_MAIL, text : FORMER_RESP_MAIL});
    });
    ...
    it('returns only valid entries from the former/additional responsible if desired',
       function() {
         ...
         fakeParentSeries.additionalResponsibles =
             [ FORMER_RESP_TEXT, FORMER_RESP_MAIL ];
         preparer.freeTextValidator =
             (text) => { return text.indexOf('@') !== -1 };
         preparer._prepareResponsibles();
         let result = preparer.getPossibleResponsibles();
         expect(result).to.have.length(4);
         expect(result).to.deep.include(
