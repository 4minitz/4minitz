...
const {
    MigrateV16
    } = proxyquire('../../../../server/migrations/migrate_v16', {
...
describe('Migrate Version 16', function () {
    ...
    describe('#up', function() {
        ...
        it('passes the topic with the correct parentId to the insert method', function() {
            MigrateV16.up();
            expect(TopicSchema.insert.firstCall.args[0].parentId).to.equal(meetingSeriesId);
            expect(TopicSchema.insert.secondCall.args[0].parentId).to.equal(meetingSeriesId);
        });
        ...
        it('updates the meeting series and removes the field topics/openTopics', function() {
            ...
            expect(MeetingSeriesSchema.update.calledWith(expectedFirstArg, expected2ndArg)).to.be.true;
        });
    ...
    describe('#down', function() {
        ...
        it('sets the fields topics/openTopics of the meeting seires correctly', function() {
            ...
            const expectedFirstArg = meetingSeriesId;
            const expected2ndArg = { $set:
                { 'openTopics': [openTopic], 'topics': [openTopic, closedTopic]} };
            expect(MeetingSeriesSchema.update.calledWith(expectedFirstArg, expected2ndArg)).to.be.true;
        });
