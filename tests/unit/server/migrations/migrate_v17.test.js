    ...
    describe('#up', function () {
        it('adds new fields to all meetingseries correctly', function() {
            ...
            MeetingSeriesSchema.find().forEach(meetingSeries => {
                ...
                const expectedMinuteStatus = meetingSeries.hasMinute && meetingSeries.lastMinuteIsFinalized ? true : false;
                
                expect(meetingSeries.lastMinutesId).to.equal(expectedMinuteId);
                expect(meetingSeries.lastMinutesFinalized).to.equal(expectedMinuteStatus);
            });
        });
    });
