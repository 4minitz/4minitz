...
const {
    MigrateV15
    } = proxyquire('../../../../server/migrations/migrate_v15', {
    './helpers/migrateItems': { MigrateItemsPre16, '@noCallThru': true}
});

describe('Migrate Version 15', function () {

    let series, minute, topicOfMinute, topicOfSeries, openTopic, actionItem1, actionItem2, actionItem3, actionItem4,
        infoItem, infoItem2;
    ...
    beforeEach(function () {
        ...
        topicOfMinute = {
            ...
            isOpen: true,
            isNew: true,
            infoItems: [infoItem, actionItem1, actionItem2, actionItem3, actionItem4, infoItem2]
        };
    ...
    const checkPriorities = (index, expectedValue) => {
        if (expectedValue === undefined) {
            expect(minute.topics[0].infoItems[index]).to.not.have.own.property('priority');
            expect(series.topics[0].infoItems[index]).to.not.have.own.property('priority');
            expect(series.openTopics[0].infoItems[index]).to.not.have.own.property('priority');
        }
        ...
        expect(series.openTopics[0].infoItems[index].priority).to.equal(expectedValue);
    };
