...
async function main() {
  try {
    ...
    let count =
        (await mongoDb.insertMany(Collections.Minutes, minutes)).insertedCount;
    ...
    console.log(
        `Inserted the meeting series "${series.project} - ${
            series.name}" with ${count} meeting minutes successfully`,
    );
