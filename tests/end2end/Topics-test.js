...
describe('Topics', function () {
  ...
  it.skip('can not change the order of topics via drag and drop by clicking anywhere', function () {
    ...
    browser.dragAndDrop(
      '#topicPanel .well:nth-child(3)',
      '#topicPanel .well:nth-child(1)'
    )
  ...
  it.skip('can not change the order of topics on the topics page', function () {
    ...
    const visibleTextBeforeSortAttempt = browser.elementIdText(
      firstElementBeforeSortAttempt
    ).value
    ...
    const visibleTextAfterSortAttempt = browser.elementIdText(
      firstElementAfterSortAttempt
    ).value
  ...
  it.skip('ensures invited user can not drag-n-drop topics', function () {
    ...
    expect(
      browser.isExisting('#topicPanel .well:nth-child(3) .topicDragDropHandle')
    ).to.be.false
  ...
  it.skip('sorting of topics is persistent', function () {
    ...
    const visibleTextBeforeReload = browser.elementIdText(
      firstElementBeforeReload
    ).value
    ...
    const visibleTextAfterReload = browser.elementIdText(
      firstElementAfterReload
    ).value
  ...
  it('can collapse a topic', function () {
    ...
    E2ETopics.addInfoItemToTopic(
      { subject: 'InfoItem#1', itemType: 'infoItem' },
      1
    )
    E2ETopics.addTopicToMinutes('topic 2')
    E2ETopics.addInfoItemToTopic(
      { subject: 'InfoItem#2', itemType: 'infoItem' },
      1
    )
    ...
    E2EGlobal.clickWithRetry(
      '#topicPanel .well:nth-child(1) #btnTopicExpandCollapse'
    )
    ...
    const firstVisibleInfoItemText = browser.elementIdText(
      firstVisibleInfoitemId
    ).value
  ...
  it('can close topics', function () {
    ...
    expect(E2ETopics.isTopicClosed(1), 'first topic should be closed').to.be
      .true
    expect(E2ETopics.isTopicClosed(2), 'second topic should be closed').to.be
      .true
  })
  ...
  it('is possible to mark topics as recurring persistently', function () {
    ...
    expect(
      E2ETopics.isTopicRecurring(1),
      'topic should not be recurring initially'
    ).to.be.false
    ...
    expect(E2ETopics.isTopicRecurring(1), 'topic should be recurring').to.be
      .true
    expect(
      E2ETopics.isTopicRecurring(2),
      'unchanged topic should not be recurring'
    ).to.be.false
  })
  ...
  it('ensures that recurring topics will be displayed as recurring even in read-only-mode', function () {
    ...
    expect(
      E2ETopics.isTopicRecurring(1),
      'recurring topic should be displayed as recurring after finalizing the minute'
    ).to.be.true
    expect(
      E2ETopics.isTopicRecurring(2),
      'unchanged topic should not be displayed as recurring after finalizing the minute'
    ).to.be.false
    ...
    expect(
      E2ETopics.isTopicRecurring(1),
      'recurring topic should be displayed as recurring on topics tab'
    ).to.be.true
    expect(
      E2ETopics.isTopicRecurring(2),
      'unchanged topic should not be displayed as recurring on topics tab'
    ).to.be.false
  })
  ...
  it('ensures that it is not possible to change the recurring flag if topic is presented in read-only-mode', function () {
    ...
    expect(
      E2ETopics.isTopicRecurring(1),
      'topic of minute should not be able to set as not-recurring if minute is finalized'
    ).to.be.true
    expect(
      E2ETopics.isTopicRecurring(2),
      'topic of minute should not be able to set as recurring if minute is finalized'
    ).to.be.false
    ...
    expect(
      E2ETopics.isTopicRecurring(1),
      'topic of meeting series should not be able to modify in topics tab'
    ).to.be.true
    expect(
      E2ETopics.isTopicRecurring(2),
      'topic of meeting series should not be able to set as recurring in topics tab'
    ).to.be.false
  })
  ...
  it(
    'ensures that the isRecurring-State of a topic in the meeting series topic list will be overwritten from the ' +
  ...
  it(
    'should not be possible to insert a new topics to a meeting minutes which has the same id as an existing one ' +
  ...
  it('add more (2) labels to topic via textbox', function () {
    ...
    E2ETopics.addTopicToMinutes(
      topicName + ' #' + labelName1 + ' #' + labelName2
    )
    E2EGlobal.waitSomeTime(500)

    expect(browser.waitForExist('.topic-labels')).to.be.true
    expect(browser.getText('.topic-labels .label:nth-child(1)')).to.equal(
      labelName1
    )
    expect(browser.getText('.topic-labels .label:nth-child(2)')).to.equal(
      labelName2
    )
  })
  ...
  it('can add a topic with more (2) labels to minutes at the end of topics list', function () {
    ...
    const labelName2 = 'testLabel2'
    E2ETopics.addTopicToMinutes('some topic on top')
    E2ETopics.addTopicToMinutesAtEnd(
      testTopicName + ' #' + labelName1 + ' #' + labelName2
    )
    E2EGlobal.waitSomeTime(500)
  ...
  it('can add a topic with responsible to minutes at the end of topics list', function () {
    ...
    const topicHeadingText = browser
      .element('#topicPanel .well:nth-child(2) h3')
      .getText()
  ...
  it('can add a topic with more (2) responsible to minutes at the end of topics list', function () {
    ...
    E2ETopics.addTopicToMinutes('some topic on top')
    E2ETopics.addTopicToMinutesAtEnd(
      testTopicName + ' @' + responsibleName1 + ' @' + responsibleName2
    )
    E2EGlobal.waitSomeTime(500)
  ...
  it('can add a topic with label and responsible to minutes at the end of topics list', function () {
    ...
    E2ETopics.addTopicToMinutes('some topic on top')
    E2ETopics.addTopicToMinutesAtEnd(
      testTopicName + ' #' + labelName + ' @' + responsibleName
    )
    E2EGlobal.waitSomeTime(500)
