...
Template.tabAdminUsers.onCreated(function() {
  this.autorun(() => { this.subscribe("onlineUsersForRoute"); });
});
...
Template.tabAdminUsers.helpers({
  users() {
    ...
    if (_showOnline.get()) {
      const onlineusers = OnlineUsersSchema.find().fetch().map(
          (ousr) => { return ousr.userId; });
...
Template.tabAdminUsers.events({
  ...
  "change #id_adminShowInactive"(
      evt,
      tmpl) { _showInactive.set(tmpl.find("#id_adminShowInactive").checked); },

  "change #id_adminShowOnline"(
      evt, tmpl) { _showOnline.set(tmpl.find("#id_adminShowOnline").checked); },
});
